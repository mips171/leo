...
// TestAddNode checks if nodes are added correctly.
...
// TestPrecede checks if edges are added correctly and detect cycles.
func TestPrecede(t *testing.T) {
    graph := TaskGraph()

    // Assume AddNode is the corrected, exported version of addNode
    graph.Add("A", func() error { return nil })
...
// TestExecutorExecute checks if the executor runs the graph correctly.
...
// You can add more tests to cover concurrency, error handling, and complex DAGs.
